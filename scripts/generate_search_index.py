...
def main():
    ...
    for fname in os.listdir(GUIDES_DIR):
        ...
        title, description, order = parse_front_matter(path)
        if not title:
        ...
        entries.append({'title': title, 'description': description, 'url': url, 'order': order or 9999})

    # Sort by order then title
    entries.sort(key=lambda e: (e.get('order', 9999), e['title'].lower()))

    # Remove order from output
    out = [{'title': e['title'], 'description': e['description'], 'url': e['url']} for e in entries]
